...
$(function(){
	...
	// $('<input type="text" value="" id="field-filter" placeholder="Find a field...">').insertBefore('#groups_list');
	...
	$('#newCustomElement').submit(function(e){
		...
		// add to the customElement array
		var inputField = $(this).find('input[type=text]');
		var element = inputField.val().replace(/\s/g, '');
		$('#custom_element_list').append('<li class="custom_element"><a href="#" class="context" data-tag="'+element.toLowerCase()+'">'+element+'<span class="remove_custom">&times;</span></a></li>');
		inputField.val('');
	});
	...
	$('body').on('click', '.remove_custom', function(e){
		e.preventDefault();
		$(this).parents('.custom_element').animate({width: 0}, 'fast', function(){ $(this).remove(); });
	});
	...
	$('body').on('click', 'a.context', function(e){
		...
		GameGenie.newContext = $(this).attr('data-tag');
		if (GameGenie.newContext === GameGenie.currentContext) { return; }

		{% verbatim -%} // jshint ignore:line
		$('section.main_body').html($('section.main_body').html().replace(
			new RegExp('([{%].*?)(' + GameGenie.currentContext + ')(.*?[%}])','g'),
			'$1' + GameGenie.newContext +'$3'
		));
		{% endverbatim -%} // jshint ignore:line

		$('a.context[data-tag="' + GameGenie.currentContext + '"]').removeClass('activeContext');
		$('a.context[data-tag="' + GameGenie.newContext + '"]').addClass('activeContext');

		GameGenie.currentContext = GameGenie.newContext;
	});
